...
const PRODUCT_LABELS: Record<ScanProduct, { label: string; description: string }> = {
  ...
  finops:  { label: 'FinOps Scan',        description: 'Cost optimization, waste detection, and rightsizing recommendations' },
...
export default function SettingsPage() {
  ...
  // Sync fetched settings into draft
  useEffect(() => {
    if (settings && !draft) {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-muted mt-0.5">Manage your organization's scan schedule</p>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-border">
          ...
          <p className="text-xs text-muted mt-1">
            Configure when each scan type runs. Scans execute daily at the specified hour (UTC).
          </p>
        </div>
