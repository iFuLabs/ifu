...
async function fetchControls(framework?: string, status?: string): Promise<Control[]> {
  const params = new URLSearchParams()
  if (framework) params.set('framework', framework)
  if (status) params.set('status', status)
  const query = params.toString() ? `?${params.toString()}` : ''
  const res = await fetch(`${API_URL}/api/v1/controls${query}`, { credentials: 'include' })
...
export default function ControlsPage() {
  ...
  const { data: controls, isLoading } = useSWR(
    ...
    () => fetchControls(framework || undefined, status || undefined),
...
function ControlRow({ control }: { control: Control }) {
  ...
  return (
    <Link
      href={`/compliance/${control.controlId}`}
