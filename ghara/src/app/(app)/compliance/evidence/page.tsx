...
export default function EvidencePage() {
  const { data, isLoading, mutate } = useSWR('evidence', () =>
    fetch(`${API_URL}/api/v1/evidence`, { credentials: 'include' }).then(r => r.json())
  )
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`${API_URL}/api/v1/evidence/${id}`, { method: 'DELETE', credentials: 'include' })
    mutate()
  }
...
function AddEvidenceForm({ onSuccess, onCancel }: { onSuccess: () => void; onCancel: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true)
    try {
      await fetch(`${API_URL}/api/v1/evidence`, {
        method: 'POST',
