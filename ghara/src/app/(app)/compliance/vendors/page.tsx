...
export default function VendorsPage() {
  const { data, isLoading, mutate } = useSWR('vendors', () =>
    fetch(`${API_URL}/api/v1/vendors`, { credentials: 'include' }).then(r => r.json())
  )
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    await fetch(`${API_URL}/api/v1/vendors/${id}`, { method: 'DELETE', credentials: 'include' })
    mutate()
  }
...
function VendorForm({ initial, onSuccess, onCancel }: { initial?: any; onSuccess: () => void; onCancel: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    setError('')
    try {
      const url = initial ? `${API_URL}/api/v1/vendors/${initial.id}` : `${API_URL}/api/v1/vendors`
